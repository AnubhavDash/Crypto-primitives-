...
public class BigIntegersOptimizations {
	...
	public static BigIntegersOptimizationsCacheKey prepareFixedBaseOptimizations(final BigInteger basis, final BigInteger modulus) {
		return prepareFixedBaseOptimizations(checkNotNull(basis), checkNotNull(modulus), BlockWidth.STANDARD);
	}

	public static BigIntegersOptimizationsCacheKey prepareFixedBaseOptimizations(final BigInteger basis, final BigInteger modulus,
			final BigIntegersOptimizations.BlockWidth blockWidth) {
		return BigIntegerOperationsService.generateCache(checkNotNull(basis), checkNotNull(modulus), checkNotNull(blockWidth));
	}
	...
	/**
	 * Represents the block width used for big integer optimizations.
	 * Each block width defines a specific value and estimated memory usage.
	 * The choice of block width impacts the performance and memory consumption
	 * of fixed-base exponentiation optimizations.
	 */
