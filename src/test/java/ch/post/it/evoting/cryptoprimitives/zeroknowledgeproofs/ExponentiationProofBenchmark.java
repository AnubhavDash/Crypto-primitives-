...
public class ExponentiationProofBenchmark {

	@Benchmark
	public ExponentiationProof genSchnorrProof(final SchnorrProofState state) {
		return state.zeroKnowledgeProofService.genExponentiationProof(state.bases, state.exponent, state.exponentiations, state.auxiliaryInformation);
	}

	@Benchmark
	public boolean verifySchnorrProof(final SchnorrProofState state) {
		return state.zeroKnowledgeProofService.verifyExponentiation(state.bases, state.exponentiations, state.proof, state.auxiliaryInformation);
	}
	...
	public static class SchnorrProofState {
		...
		@Setup(Level.Trial)
		public void setup() {
			bases = randomService.genRandomVector(gqGroup.getQ(), size).stream()
					.map(element -> GqElement.GqElementFactory.fromSquareRoot(element.getValue(), gqGroup))
					.collect(GroupVector.toGroupVector());
			exponent = ZqElement.create(randomService.genRandomInteger(gqGroup.getQ()), ZqGroup.sameOrderAs(gqGroup));
			exponentiations = bases.stream()
			...
			proof = zeroKnowledgeProofService.genExponentiationProof(bases, exponent, exponentiations, auxiliaryInformation);
		}
	}
